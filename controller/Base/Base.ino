...
void messageHandler(){
  ...
  switch (message[1]){
    ...
    case 'S':
      ...
      break;
    case 'R':
      Serial.print("RF Bridge = ");
      buf[0] = 1;  // add the Start of Transmission ascii
      //buf[1] = 'T';  // T for Temp
      for(int i=2; i <= msgPlace; i++){
        Serial.print(message[i]);
        buf[i-2] = (uint8_t)message[i];
      }
      Serial.println("");
      if (manager.sendtoWait(buf, sizeof(buf), ADDR_ALL)){
...
void sendTime(){
  ...
  manager.sendto(buf, sizeof(buf), ADDR_ALL);
}
